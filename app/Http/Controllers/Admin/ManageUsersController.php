...
public function dormant($id)
{
    \App\Models\User::where('id', $id)->update([
        'status' => 'inactive', // Use status
        'account_status' => 'inactive' // Update both just in case
    ]);
...
public function undormant($id)
{
    \App\Models\User::where('id', $id)->update([
        ...
        'account_status' => 'active' // Update both just in case
    ]);

    return redirect()->back()->with('success', 'Action Successful! Account reactivated.');
}
